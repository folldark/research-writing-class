...
目前單元：**2026-09-09「問題意識」再探，共 38 張投影片**。
...
4. 依內容選用 `title-slide`、`section-slide`、`statement-slide`、`question-slide`；主題色可用 `navy`、`slate`、`mist`、`paper`。長句可用 `display-copy` 或 `reading-title`，優先拆頁。講者提示放在 `<aside class="speaker-notes">`，不擴寫原稿正文；沒有實質提示就不要放空泛的罐頭備註。
